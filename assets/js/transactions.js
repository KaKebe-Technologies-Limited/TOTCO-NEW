...
const TransactionsReportDatatable = function () {
    ...
    const transactions = function () {
        ...
        const transaction-categoryElement = document.getElementById('transaction-category');
        ...
        let datatable = $('#transaction-table').DataTable({
            ...
            ajax: {
                url: `/interim/transactionReports?startdate=${moment().subtract(29, 'days').format('YYYY-MM-DD')}&enddate=${moment().format('YYYY-MM-DD')}&status=${transactionStatusElement.value}&businessId=${businessId}&product=${transaction-categoryElement.value ?? "ALL"}`,
                method: 'post',
            },
            //order: [[1, "desc"]],
            columns: [
        ...
        $('input[name="DateRage"]').daterangepicker({
            ...
            datatable.ajax.url(`/interim/TransactionReports?startdate=${start.format('YYYY-MM-DD')}&enddate=${end.format('YYYY-MM-DD')}&status=${transactionStatusElement.value}&product=${transaction-categoryElement.value ?? "ALL"}`).load();
        ...
        transactionStatusElement.addEventListener('change', function (event) {
            ...
            const product = transaction-categoryElement.value ?? "ALL";

            datatable.ajax.url(`/interim/TransactionReports?startdate=${start}&enddate=${end}&status=${status}&product=${product}`).load();
        });

        transaction-categoryElement.addEventListener('change', function (event) {
            event.preventDefault();
    ...
    const exportExcel = function () {
        ...
        btn.addEventListener('click', e => {
            ...
            fetch(`${window.location.origin}/transactions/transactionExcelExport?startdate=${startDate}&enddate=${endDate}&status=${transactionStatusElement.value}&businessId=${businessId}&product=${transaction-categoryElement.value ?? "ALL"}`)
    ...
    const exportPdf = function () {
        ...
        btn.addEventListener('click', e => {
            ...
            fetch(`${window.location.origin}/transactions/transactionExportPDf?startdate=${startDate}&enddate=${endDate}&status=${transactionStatusElement.value}&businessId=${businessId}&product=${transaction-categoryElement.value ?? "ALL"}`)
                .then(response => response.json())
                .then(result => {
                    btn.innerHTML = text;
                    btn.removeAttribute('disabled');
